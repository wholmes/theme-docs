# Foundry Gallery — Theme User Guide

A complete guide for gallery staff, site administrators, and editors using the **Foundry Gallery** WordPress theme. This document explains how to install the theme, structure content, manage the homepage, and troubleshoot common issues.

**Theme folder:** `foundry-gallery` (install path)  
**Appearance name:** Foundry Gallery  
**Current version:** 1.3.25 (`FOUNDRY_THEME_VERSION` in `functions.php`; sync `style.css` Version)
...
27. [For developers and agencies](#27-for-developers-and-agencies)
28. [Related documentation](#28-related-documentation)
29. [Distributing or re-branding the theme](#29-distributing-or-re-branding-the-theme)
...
See also `foundry-gallery/README.md` for cPanel-specific troubleshooting.

### Staging → production

Use `docs/MIGRATION.md` and the scripts in `scripts/` for database and media migration. After migration:

- **ACF → Sync** if field JSON changed
...
Catalog reference: `docs/SEED-CONTENT.md`  
PHP source: `foundry-gallery/inc/seed-data.php`
...
**Recommended Rank Math settings:** see `docs/RANK-MATH.md`
...
## 27. For developers and agencies

### Child themes and hooks

Inject markup without editing core templates using action hooks (`foundry_before_footer`, `foundry_home_after_section_video`, etc.). Full list: `docs/HOOKS.md`

Register custom homepage section layouts via `foundry_home_section_layouts` filter and `template-parts/home/sections/{layout}.php`.

### ACF Local JSON

Field groups live in `foundry-gallery/acf-json/`. Edit fields in WP admin and sync, or edit JSON in version control.
...
| CPTs / taxonomies | `inc/cpt.php` |
| Seed catalog | `inc/seed-data.php` |
| Demo importer | `inc/seed-content.php` |
| Homepage sections | `inc/home-sections.php`, `inc/home-sections-admin.php` |
| Header auto-hide | `assets/js/header-auto-hide.js`, `inc/site-settings.php` |
| Content pages | `page.php`, `page-content.php`, `template-parts/page/standard.php`, `assets/css/editor.css` |
| Hero title animation | `inc/hero.php`, `template-parts/hero/section.php` |
| Images / fallbacks | `inc/images.php` |
| Forms | `inc/forms.php` |
| SEO | `inc/seo.php`, `inc/rank-math.php` |
| Site settings / notices | `inc/site-settings.php` |
| Smooth scroll | `assets/js/smooth-scroll.js`, `inc/enqueue.php` |
| Brand defaults | `inc/brand.php` |

### ACF Free vs Pro

| Feature | Free approach in this theme |
|---------|----------------------------|
| Hero slides | `hero_slide` CPT |
| Staff / locations | CPTs |
| Site-wide settings | Page `site-settings` (Foundry Settings in admin) |
| Page display | Sidebar on **Pages** — hero animation, **Content width** |
| Content pages | Block editor (Gutenberg) on Default + Content Page; disabled on Home + Foundry Settings |
| Homepage sections | Section builder meta box + ACF tabs (visibility, content) on Home page |
| Page hero animation | `true_false` on Pages + archive toggles on Foundry Settings |
| Galleries | Media attachments + fallbacks |

### Static HTML prototypes

Reference markup in the repo root (`index.html`, `artist-page-improved.html`, etc.) — not used by WordPress at runtime.
...
| Document | Contents |
|----------|----------|
| `foundry-gallery/README.md` | Install, Namecheap deploy, quick reference |
| `docs/SEED-CONTENT.md` | Fictional demo catalog (artists, exhibitions, works) |
| `docs/HOOKS.md` | Theme action hooks and examples |
| `docs/RANK-MATH.md` | Rank Math compatibility |
| `docs/MIGRATION.md` | Staging → production migration |
| `docs/DISTRIBUTION.md` | Theme product vs site brand; white-label checklist |

---

## 29. Distributing or re-branding the theme

Editors rebrand the **live site** via **Foundry Settings** and content — see [§7](#7-foundry-settings-page).

Developers shipping the theme as a **product** (agency kit, client handoff, marketplace):

| Goal | Action |
|------|--------|
| Your name in **Appearance → Themes** | Edit `style.css` → `Author`, `Theme URI`, `Theme Name` |
| New gallery demo defaults | Edit `inc/brand.php` → `foundry_brand_config()` or filter from a child theme |
| Client site without code | Foundry Settings + replace demo content |
| Stable updates | Keep folder slug `foundry-gallery` and text domain unchanged |

Full guide: **`docs/DISTRIBUTION.md`**

| Document | Contents |
|----------|----------|
| `README.md` (repo root) | Repository overview |
